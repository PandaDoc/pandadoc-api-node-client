...
import { DocumentCreateSectionByTemplateRequest } from '../models/DocumentCreateSectionByTemplateRequest';
...
export class DocumentsApiRequestFactory extends BaseAPIRequestFactory {
    ...
    /**
     * Create document section by template
     * @param documentId Document ID for the section to be added to
     * @param documentSectionCreateRequest Use a PandaDoc template to create a section on an existing document 
     */
    public async createDocumentSectionByTemplate(documentId: string, documentSectionCreateRequest: DocumentCreateSectionByTemplateRequest, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'documentCreateRequest' is not null or undefined
        if (documentSectionCreateRequest === null || documentSectionCreateRequest === undefined) {
            throw new RequiredError("DocumentsApi", "createDocumentSection", "documentSectionCreateRequest");
        }

        // Path Params
        const localVarPath = '/public/v1/documents/{id}/sections/uploads'
        .replace('{' + 'id' + '}', encodeURIComponent(String(documentId)));
        ...
        requestContext.setHeaderParam("Content-Type", contentType);
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(documentSectionCreateRequest, "DocumentCreateSectionByTemplateRequest", ""),
            contentType
        );
        requestContext.setBody(serializedBody);
